...
import { Calendar, MapPin, Clock, Lock, Loader2, CheckCircle2, AlertTriangle } from 'lucide-react';
        ...
        <div className="min-h-screen bg-slate-50 flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-4 text-center">
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 max-w-md w-full">
                    ...
                    <p className="text-slate-500 mb-6">This event has concluded or the link is invalid.</p>
            ...
            <div className="container mx-auto px-4 md:px-6 relative z-20 py-24 md:py-32 max-w-5xl text-center md:text-left">
                ...
                <div className="flex flex-col md:flex-row gap-6 text-lg text-slate-200">
                    <div className="flex items-center gap-2 justify-center md:justify-start">
                        ...
                        <span>{event.date ? new Date(event.date).toLocaleDateString(undefined, { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : 'TBD'}</span>
                    </div>
                    <div className="flex items-center gap-2 justify-center md:justify-start">
                    ...
                    <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-6 sticky top-24">
                        {registered ? (
                            <div className="text-center py-8">
                                ...
                                <button onClick={() => setRegistered(false)} className="text-sm text-indigo-600 font-bold hover:underline">Register another guest</button>
                            </div>
                        ) : (
                            <form onSubmit={handleRegister} className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input required type="text" className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Jane Doe" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-1">Email Address</label>
                                    <input required type="email" className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="jane@company.com" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-1">Ticket Type</label>
                                    <select className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none bg-white" value={formData.ticketType} onChange={e => setFormData({...formData, ticketType: e.target.value})}>
                                        <option value="General">General Admission - Free</option>
                                        <option value="VIP">VIP Access</option>
                                        <option value="Press">Media / Press</option>
                                    </select>
                                </div>
                                <button type="submit" disabled={registering} className="w-full py-3.5 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-600/20 disabled:opacity-70 flex items-center justify-center gap-2">
