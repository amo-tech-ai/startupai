...
const Sidebar: React.FC = () => {
  ...
  const menuItems = [
    ...
    { path: '/agents', icon: <Bot size={20} />, label: 'Agent Hub' },
  ...
  return (
    <aside className="w-64 bg-slate-900 text-white flex flex-col h-screen sticky top-0 border-r border-slate-800 shadow-xl z-20 hidden md:flex">
      ...
      <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto custom-scrollbar">
        {menuItems.map((item) => {
          const isActive = location.pathname === item.path || (item.label === 'Events' && location.pathname.startsWith('/events')) || (item.label === 'Agent Hub' && location.pathname.startsWith('/agents'));
          return (
            <button
              key={item.path}
