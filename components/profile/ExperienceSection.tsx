...
import { Plus, Briefcase, Calendar, Building2, Trash2 } from 'lucide-react';
...
export const ExperienceSection: React.FC<ExperienceSectionProps> = ({ experiences, onUpdate }) => {
  ...
  const [newExp, setNewExp] = useState<Partial<UserProfileExperience>>({});
  ...
  const handleSaveNew = () => {
    if (newExp.company && newExp.role) {
      const entry: UserProfileExperience = {
        ...
        role: newExp.role,
        startDate: newExp.startDate || '2024',
        endDate: newExp.current ? 'Present' : (newExp.endDate || ''),
      ...
      {isAdding && (
        <div className="mb-8 p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-4 animate-in fade-in slide-in-from-top-2">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button onClick={handleSaveNew} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">Save</button>
          </div>
        </div>
      )}
